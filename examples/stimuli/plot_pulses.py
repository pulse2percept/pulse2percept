...
as either anodic first or cathodic first. Multiple pulses can form a pulsetrain.
...
:py:class:`~pulse2percept.stimuli.Stimulus` is the base class to generate
 different types of stimulus. Simplest way to instantiate a Stimulus is to pass
a scalar value which is interpreted as the current amplitude for a single
electrode.
...
#
# * ``electrodes``: when a single scalar value is passed, there
