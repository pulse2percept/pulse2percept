...
-  A naming convention ``names`` for the rows and columns in the grid.
   ...
   ``('A', 'A')``.
-  An electrode type ``electrode_type``, which must be a subclass of
...
-  Any additional parameters that should be passed to the
   ...
   ``radius`` for :py:class:`~pulse2percept.implants.DiskElectrode`.
...
# we need to explicitly specify the electrode type (``electrode_type``) and
# the ``radius`` to use:


# 11x13 grid, 100-um disk electrodes spaced 500um apart:
disk_grid = ElectrodeGrid((11, 13), 500, electrode_type=DiskElectrode,
                          radius=100)
...
hex_grid = ElectrodeGrid((11, 13), 500, grid_type='hex',
                         electrode_type=DiskElectrode, radius=100)
...
offset_grid = ElectrodeGrid((11, 13), 500, grid_type='hex', x=-600, y=200,
                            z=150, rot=-45, electrode_type=DiskElectrode,
                            radius=100)
...
implant = GridImplant((11, 13), 500, grid_type='hex',
                      electrode_type=DiskElectrode, radius=100)
implant.plot()
