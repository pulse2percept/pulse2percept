...
class Stimulus(PrettyPrint):
    ...
    200 ms, with 0.45ms pulse duration, 40uA current amplitude, and the
    time series resolved at 0.1ms resolution:

    >>> from pulse2percept.stimuli import Stimulus, PulseTrain
    >>> stim = Stimulus({'B9': PulseTrain(0.1, freq=10, amp=40, dur=200)})

    Compress an existing Stimulus:

    >>> from pulse2percept.stimuli import Stimulus, PulseTrain
    >>> stim = Stimulus({'B9': PulseTrain(0.1, freq=10, amp=40, dur=500)})
    >>> stim.shape
    (1, 5000)
    >>> stim.compress()
    >>> stim.shape
...
@deprecated(alt_func=Stimulus, deprecated_version='0.6', removed_version='0.7')
